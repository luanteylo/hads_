...
CREATE TABLE public.execution (
    ...
    task_max_memory double precision,
...
CREATE TABLE public.instance_status (
    instance_id character varying NOT NULL,
    "timestamp" timestamp without time zone NOT NULL,
    memory_footprint double precision,
    cpu_usage double precision,
    cpu_credit double precision,
    status character varying
);
...
COPY public.execution (execution_id, job_id, task_id, instance_id, "timestamp", task_max_memory, status) FROM stdin;
\.
...
COPY public.instance_status (instance_id, "timestamp", memory_footprint, cpu_usage, cpu_credit, status) FROM stdin;
\.
...
COPY public.instance_type (type, vcpu, memory, provider, burstable) FROM stdin;
\.
...
COPY public.statistic (execution_id, job_id, start, "end", deadline, cost) FROM stdin;
\.
...
ALTER TABLE ONLY public.statistic
    ADD CONSTRAINT statistic_pkey PRIMARY KEY (execution_id, job_id);
